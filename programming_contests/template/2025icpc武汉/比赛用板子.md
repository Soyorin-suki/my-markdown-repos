...
struct SegmentTree{
	...
	ll build(ll l,ll r){
		ll now=++cnt;
		if(l==r){tr[now].sum=0;return now;}
		ll mid=(l+r)>>1;
		ls(now)=build(l,mid);
	...
	ll update(ll last,ll nowl,ll nowr,ll pos,ll k){
		ll now=++cnt;
		tr[now]=tr[last];
		if(nowl==nowr){
			tr[now].sum+=k;
		...
		ll mid=(nowl+nowr)>>1;
		if(pos<=mid)ls(now)=update(ls(last),nowl,mid,pos,k);
		else rs(now)=update(rs(last),mid+1,nowr,pos,k);
		push_up(now);
		return now;
	}
	ll query(ll last,ll now,ll nowl,ll nowr,ll k){
		if(nowl==nowr)return nowl;
		ll lsum=0;
		if(ls(now))lsum+=tr[ls(now)].sum;
		if(ls(last))lsum-=tr[ls(last)].sum;
		ll mid=(nowl+nowr)>>1;
		if(lsum>=k)return query(ls(last),ls(now),nowl,mid,k);
		else return query(rs(last),rs(now),mid+1,nowr,k-lsum);
	}
	void init(ll n){
		this->n=n;
		cnt=0;
...
**线段树上二分**
来自U502676，查询`[l,r]`上第一个大于k的数的下标。如果不存在这样的数，返回-1
```c++
struct SgementTree{
	ll tree[N<<2];
	...
	ll query(ll p,ll nowl,ll nowr,ll l,ll r,ll v){
		if(tree[p]<=v)return -1;
		if(nowl==nowr){
			return tree[p]>v?nowl:-1;
		}
		ll mid=(nowl+nowr)>>1;
		if(l<=nowl&&nowr<=r){
			if(tree[ls(p)]>v)return query(ls(p),nowl,mid,l,r,v);
			else return query(rs(p),mid+1,nowr,l,r,v);
		}else{
			ll res=-1;
			if(l<=mid)res=query(ls(p),nowl,mid,l,r,v);
			if(res==-1&&mid+1<=r)res=query(rs(p),mid+1,nowr,l,r,v);
			return res;
		}
	}
}t;
...
struct SegmentTree{
	...
	inline ll ls(ll x){return x<<1;}
	inline ll rs(ll x){return x<<1|1;}
	ll query(ll p,ll nowl,ll nowr,double v){
		if(nowl==nowr)return tr[p].mx>v;
		else if(tr[p].mx<=v)return 0;
		ll mid=(nowl+nowr)>>1;
		if(tr[ls(p)].mx>v)return query(ls(p),nowl,mid,v)+tr[p].ans-tr[ls(p)].ans;
		else return query(rs(p),mid+1,nowr,v);
	}
	void pushUp(ll p,ll l,ll r){
		ll mid=(l+r)>>1;
		tr[p].mx=max(tr[ls(p)].mx,tr[rs(p)].mx);
		tr[p].ans=tr[ls(p)].ans+query(rs(p),mid+1,r,tr[ls(p)].mx);
	}
	void build(ll p,ll l,ll r){
		tr[p].ans=0;tr[p].mx=0;
		if(l==r)return;
		ll mid=(l+r)>>1;
		build(ls(p),l,mid);
		build(rs(p),mid+1,r);
	}
	void update(ll p,ll nowl,ll nowr,ll pos,double k){
		if(nowl==nowr){
			tr[p].ans=1;
			tr[p].mx=k;
			return;
		...
		ll mid=(nowl+nowr)>>1;
		if(pos<=mid)update(ls(p),nowl,mid,pos,k);
		else update(rs(p),mid+1,nowr,pos,k);
		pushUp(p,nowl,nowr);
	}
}t;
```
...
struct Dinic{
	...
	ll s,t,n;
	ll bfs(){
		for(ll i=1;i<=n+m+2;++i)dis[i]=INF;
		queue<ll>q;
		q.push(s);
...
```c++
struct MCMF{
	vector<ll>now,dis,vis;
	queue<ll>q;
	ll max_flow,min_cost,s,t,n;
	ll spfa(){
		for(ll i=1;i<=n;++i){
			dis[i]=INF;vis[i]=0;
			now[i]=head[i];
		...
		while(!q.empty()){
			ll u=q.front();q.pop();
			for(ll i=head[u];i;i=e[i].nxt){
				ll v=e[i].to,cap=e[i].cap,cost=e[i].cost;
	...
	ll dfs(ll u,ll sum){
		...
		for(ll i=now[u];i;i=e[i].nxt){
			now[u]=i;
			ll v=e[i].to,cap=e[i].cap,cost=e[i].cost;
			if(vis[v]||cap==0||dis[v]!=dis[u]+cost)continue;
			k=dfs(v,min(sum,cap));
			if(k==0)dis[v]=INF;
			e[i].cap-=k;
			e[i^1].cap+=k;
	...
	pll mcmf(ll s,ll t,ll n){
        this->s=s;this->t=t;this->n=n;
		dis.resize(n+1,0),now.resize(n+1,0),vis.resize(n+1,0);
...
void add(ll u){
	cnt[color[u]]++;
	if(cnt[color[u]]==1)nowans++;
}
void del(ll u){
	cnt[color[u]]--;
	if(cnt[color[u]]==0)nowans--;
}
void addSubtree(ll fa,ll u){
	add(u);
	for(ll v:e[u]){
		if(v==fa)continue;
		addSubtree(u,v);
...
void dfs(ll fa,ll u,bool keep=1){
	for(ll v:e[u]){
		if(v==fa||v==son[u])continue;
		dfs(u,v,0);
	}
	if(son[u])dfs(u,son[u],1);
	add(u);
	for(ll v:e[u]){
		if(v==fa||v==son[v])continue;
		addSubtree(u,v);
	}
	ans[u]=nowans;
	if(keep==0){
...
ll lca(ll u,ll v){
	if(dep[u]<dep[v])swap(u,v);
	for(ll i=20;i>=0;--i){
		if(dep[faz[u][i]]>=dep[v])u=faz[u][i];
	}
	if(u==v)return u;
	for(ll i=20;i>=0;--i){
		if(faz[u][i]!=faz[v][i])u=faz[u][i],v=faz[v][i];
	}
	return faz[u][0];
}
vector<ll>virE[N];
ll query[N];
vector<ll>node;
inline bool isAncenstor(ll fa,ll u){
	return tin[fa]<=tin[u]&&tout[fa]>=tout[u];
}
bool cmp(ll a,ll b){
	return tin[a]<tin[b];
}
void build(vector<ll>&node){
	...
	set<ll>node_st;
	for(ll u:node)node_st.insert(u);
	for(ll i=1;i<node.size();++i){
		node_st.insert(lca(node[i],node[i-1]));
	}
	node.clear();for(ll u:node_st)node.push_back(u);
	sort(node.begin(),node.end(),cmp);
	vector<ll>stk;
	for(ll v:node){
		while(!stk.empty()&&!isAncenstor(stk.back(),v)){
			stk.pop_back();
		}
		if(!stk.empty()){
			virE[stk.back()].push_back(v);
		}
		stk.push_back(v);
	}
}
...
void solve(void){
	...
	for(ll i=1;i<=cnt;++i){
		...
		tmp=tmp*invfact[i-1]%MOD*invfact[cnt-i]%MOD;
...
// 欧拉筛求phi
ll prime[N],vis[N],cnt,phi[N];
ll hasRoot[N];// 是否有原根，当n=1,2,4,p^k,2p^k时存在原根
void seive(ll n){
	for(ll i=2;i<=n;++i){
		if(!vis[i])prime[++cnt]=i,phi[i]=i-1;
		for(ll i2=1;i2<=cnt&&prime[i2]*i<=n;++i2){
			vis[i*prime[i2]]=1;
			if(i%prime[i2]==0){
				phi[i*prime[i2]]=phi[i]*prime[i2];
				break;
			}
			phi[i*prime[i2]]=phi[i]*(prime[i2]-1);
		}
	}
	for(ll i=2;i<=cnt;++i){
		ll now=prime[i];
		while(now<=n){
			hasRoot[now]=prime[i];
			if(now*2<=n)hasRoot[now*2]=prime[i];
			now*=prime[i];
		}
	}
	hasRoot[2]=hasRoot[4]=1;
}
vector<ll>fact;// 对phi[n]质因数分解
vector<ll> getPrime(ll x){
	vector<ll>p;
	for(ll i=1;prime[i]*prime[i]<=x&&i<=cnt;++i){
		if(x%prime[i]==0){
			p.push_back(prime[i]);
...
ll check(ll x,ll p){
	for(ll k:fact){
		if(q_pow(x,phi[p]/k,p)==1)return 0;
	}
	return 1;
}
// 获得最小原根。一个数的最小原根是很小的，可以遍历得到
ll getMinRoot(ll n){
	if(n==2)return 1;
	if(n==4)return 3;
	for(ll i=1;i<n;++i){
...
// 已知n的一个原根g，获取所有原根
vector<ll> getRoot(ll n,ll g){
	vector<ll>ans;
	if(n==2)return {1};
	if(n==4)return {3};
	for(ll now=g,i=1;i<phi[n];++i,now=now*g%n){
		if(gcd(i,phi[n])==1)ans.push_back(now);
	}
...
ll bsgs(ll a,ll b,ll p){
	...
	for(ll i=1;i<=t;++i){
		if(mp.count(now)){
			if(i*t-mp[now]>=0)return i*t-mp[now];
		}
		now=now*a%p;
	}
	return -1;
...
ll exbsgs(ll a,ll b,ll p){
	...
	if(b%d!=0)return -1;
	ll ans=exbsgs(a,b/d*inv(a/d,p/d)%(p/d),p/d);
	return ans+(ans!=-1);
}
```
**普通bsgs**
...
for(int i=0;i<n;++i){
    for(int i2=0;i2<(1<<n);++i2){
        if(i2&(1<<i))f[i2]+=f[i2^(1<<i)];
    }
}
```
因此，将数质因数分解后，狄利克雷前缀和也可以看做一个高维前缀和，可以边筛边做
...
GPT给出，未经测验
```c++
// f: 求值函数（整数输入，返回可比较类型，例如 long long 或 double）
// 在 [L, R] 闭区间上假设 f 是单峰（先增后减），求 argmax 或 max value 的索引/值。
int ternary_max_index(int L, int R) {
    int l = L, r = R;
    while (r - l > 3) {                     // 保证最后剩下小常数个点用暴力检查
        int m1 = l + (r - l) / 3;
        int m2 = r - (r - l) / 3;
        if (f(m1) < f(m2)) {
            l = m1 + 1;    // 峰在右半区（含 m2）
    ...
    // 最后暴力枚举 [l, r]
    int best = l;
    for (int i = l+1; i <= r; ++i) if (f(i) > f(best)) best = i;
    return best;
}
```
**浮点数三分**
...
造好check.sh后，在终端输入`chmod 777 check.sh`（相当于取得使用权限）然后就可以输入`./check.sh`来愉快的对拍了。

另外，在终端输入`time a<data.in>data.out`可以查看你的程序跑了多少时间。
...
```
```c++
uniform_int_distribution<int> dist(1, 100); // [1,100] 均匀分布
for (int i = 0; i < 5; ++i)
        cout << dist(rng) << " ";
